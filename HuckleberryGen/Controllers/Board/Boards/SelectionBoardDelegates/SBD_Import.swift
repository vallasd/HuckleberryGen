...
class SBD_Import: SelectionBoardDelegate {
    ...
    func parse(importFile: ImportFile) {
        ...
        if parser == nil {
            appDelegate.error.report("Project Importer was unable to create parser from importFile \(importFile)", type: .Error)
        }
    }
...
extension SBD_Import: HGImportParserDelegate {
    ...
    func parserDidParse(importFile: ImportFile, success: Bool, project: Project) {
        if success {
            ...
            appDelegate.error.report("Import Error: could not parse import file: \(importFile.name)" , type: .Error)
        }
    }
}
