...
enum ProgressionType {
    ...
    init(int: Int) {
        ...
        default:
            HGReportHandler.report("Int: |\(int)| is not ProgressionType mapable, returning .Next", type: .Error)
            self = .Next
        }
    }
...
enum RegressionType {
    ...
    init(int: Int) {
        ...
        case 802: self = .Home
        default:
            HGReportHandler.report("Int: |\(int)| is not RegressionType mapable, returning .Cancel", type: .Error)
            self = .Cancel
        }
    }
...
class NavController: NSViewController {
    ...
    private func setCurrentVC(withBoardData boardData: BoardData) {
        ...
        if currentVC == nil {
            HGReportHandler.report("Nav Controller, currentVC was not properly instantiated from BoardData", type: .Error)
            return
        }
        ...
        if let context = boardData.context {
            if let saveableVC = currentVC as? BoardRetrievable {
                ...
                HGReportHandler.report("Nav Controller, attempting to set context for NSViewController that is not BoardRetrievable", type: .Error)
            }
        }
...
extension NavController {
    ...
    func string(fromTag tag: Int) -> String {
        ...
        HGReportHandler.report("\(tag) is Not A Regression or Progression Type Tag", type: .Error)
        return ""
    }
